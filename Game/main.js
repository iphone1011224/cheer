 ...
 ctx.drawImage(imgMain,0,0,80,130,currentImgMainX,currentImgMainY,200,200);
    ...
    imgMountain.onload=function(){
        ...
        for(var x in mapArray)
        {
            ...
            {
 ctx.drawImage(imgMountain,320,65,32,32,x%3*200,Math.floor(x/3)*200,200,200);
            }else if(mapArray[x]==3)//擺上敵人
            {
ctx.drawImage(imgMountain,160,65,32,32,x%3*200,Math.floor(x/3)*200,200,200); 
        }
        }
        };};
});
...
$(document).keydown(function(event){
    ...
    //targetImgX,targetImgY:主角即將要移動過去的目標位置
    //targetBlock:主角即將要移動過去的那一格編號
    //cutImagePositionX:依據主角方向而決定的圖片
